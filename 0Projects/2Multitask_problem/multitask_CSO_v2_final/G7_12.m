...
%     g(8) = -3*x(1)+6*x(2)+12*(x(9)-8)^2-7*x(10); 
    ...
    g(1) = 4*x(1)+5*x(2)-3*x(7)+9*x(8)-105 + x(11) - 1;
    g(2) = 10*x(1)-8*x(2)-17*x(7)+2*x(8) + x(12) + 1;    
    g(3) = -8*x(1)+2*x(2)+5*x(9)-2*x(10)-12 + x(13) - 1; 
    g(4) = 3*(x(1)-2)^2+4*(x(2)-3)^2+2*x(3)^2-7*x(4)-120 + x(14) + 1;      
    g(5) = 5*x(1)^2+8*x(2)+(x(3)-6)^2-2*x(4)-40 + x(15) - 1;
    g(6) = 0.5*(x(1)-8)^2+2*(x(2)-4)^2+3*x(5)^2-x(6)-30 + 1; 
    g(7) = x(1)^2+2*(x(2)-2)^2-2*x(1)*x(2)+14*x(5)-6*x(6) - 1;       
    g(8) = -3*x(1)+6*x(2)+12*(x(9)-8)^2-7*x(10) + 1; 
end
